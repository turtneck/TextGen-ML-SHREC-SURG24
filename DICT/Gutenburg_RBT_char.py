...
try:
    for txtpath in dirlist:
        ...
        logger(printpath+f'gutenburg_log-RBT-chr.txt',   f"{t_str}{'.'*(55-len(t_str))}\t+<{word_cnt}> chars\t{  goodtime(nowtime-start_time)  }\t<{   goodtime(nowtime-script_time)   }> RUNTIME\t{last_word}")
        cnt+=1
