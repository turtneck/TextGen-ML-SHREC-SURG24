...
for i in range(start,file_num):
    ...
    time.sleep(waits)
    sys.stdout.write(f"{Fore.CYAN}PROG {i}: <{gdFL( 100*i/file_num )}%>  ");pr=False
    for j in range(10):
        try:
            ...
            if element.text == "English":
                ...
                curr_size += size
                prGreen(f"DL: {gdFL( size/(10**6) )}MB: <{gdFL( 100*curr_size/(limit*(10**9)) )}% full>    {download_dir+f'/gutenburg/pq{i}.txt'}   {Fore.LIGHTYELLOW_EX}T:{gdFL( t )}");pr=True
